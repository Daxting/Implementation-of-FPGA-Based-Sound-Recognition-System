...
module ShuffleNet_FSM(
    ...
    */
    ...
    always_comb begin
        if(Cal_State == CAL_STATE::CALC || Cal_State == CAL_STATE::MPOL) begin
            case(Stage_Type)
                ...
                STAGE_TYPE::DWS2: begin
                    if(Stage == STAGE::GLOBAL_DW) begin
                        Write_Require = MPB_Out_Ready & (Feature_In_Y != 7'd3);
                        case(Operate)
    ...
    always_comb begin
        if(Stage == STAGE::GLOBAL_DW) begin
            if(next_Cal_State == CAL_STATE::CALC || next_Cal_State == CAL_STATE::MPOL) begin
                if(Step_Carry & !(next_Feature_In_Y[0])) begin
    ...
    always_comb begin
        if(Stage == STAGE::GLOBAL_DW) begin
            ...
            if(!(Feature_In_Y[0])) begin
                if((Cal_State == CAL_STATE::CALC | Cal_State == CAL_STATE::MPOL) & (MPB_In_Sel[0] == 1'b1)) begin
                    case(Step_Cnt)
